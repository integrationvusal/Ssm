...
function hasCurrency(currency_id){
    for(let x in localcurrencies){
       for(let y in localcurrencies[x]){
          if(localcurrencies[x][y] == currency_id) return localcurrencies[x].name;
       }
    }
}

function getCurrency(name, field){
	
    for(let x in localcurrencies){
		if(localcurrencies[x].name.toLowerCase() == name) return localcurrencies[x][field];
    }
}
...
$(document).ready(function() {
    ...
    $('.hidemenu, .showmenu').click(function(e){
    	 _target = e.target.classList[e.target.classList.length-1];
        $('.sidebar')[animationMenuParams[_target]['action']]('anim-fade');
        $('.main').attr('class', mainContainerClasses[animationMenuParams[_target]['class']]);
        setTimeout(function(){
            $('.showmenu')[animationMenuParams[_target]['arrow']]();
        }, 500);
    });
    ...
    $("[name='sell_code_switcher']").bootstrapSwitch({
		...
		onSwitchChange:function(e, s){
			if(!s){
				sessionStorage.removeItem('good_search');
			}
		}
    });
	
	if(sessionStorage.hasOwnProperty('good_search') && $('#sell_goods_code').attr('id') !== undefined){
		setTimeout(function(){
			$('.bootstrap-switch-handle-off').trigger('click');
			$('#goods_code').val(sessionStorage.getItem('good_search'));
			$('#sell-search-goods').trigger('click');
		}, 300);
	}
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(response.status > 0){
					...
					sessionStorage.setItem('good_search', $('#goods_code').val());
					location.reload();
    ...
    function changePriceCount(event){
        ...
        $("#pending-total-"+(localcurrencies[currency].name.toLocaleLowerCase()) ).html((parseFloat(count) * parseFloat(price)).toFixed(2));
    ...
    function calcSell(){
        var total = [];
        $(".pending-total").each(function(key, val){
            _val = parseFloat($(this).html());
           if(_val>1){
	           _currency = $(this).attr('data-currency');
                total[_currency] = (total[_currency] || 0) + _val;
           }
        });
        
        for(x in total)
            $(".total_sell_price_"+localcurrencies[x].name.toLowerCase()).html(total[x].toFixed(2));
    ...
    $(".sell-approve").unbind().click(function(event){
        ...
        $('[name="received_payment"]').each(function(index){
            ...
            _eachData[_target] = {};
            _eachData[_target]['currency'] = getCurrency(_target, 'id');
            _eachData[_target]['currency_archive'] = getCurrency(_target, 'value');
            _eachData[_target]['received_payment'] = $(this).val();
            _eachData[_target]['client'] = $('#client_'+_target).val();
            _eachData[_target]['debtamount'] = $('#payed_'+_target).val() || 0;
            _eachData[_target]['cash'] = $('[name="cash_'+_target+'"]').val();
            _eachData[_target]['amount'] = $(".total_sell_price_"+_target).html();
            _eachData[_target]['invoice_id'] = index?0:$("#invoice_id").val();
            _eachData[_target]['invoice_serial'] = $(this).parents('.dl-horizontal').find('dd:first').text();
            _eachData[_target]['ids'] = $.map($('#sell-pendings tr[data-currency="'+_target+'"]'), function(val){ return val.id;});
        });
       ...
       // data.debtamount = (data.debtamount > 0 && data.debtamount_azn > 0)?data.debtamount:(data.debtamount>0?data.client:data.debtamount_azn);
        ...
        if(_data.cash == 2){
            if(!submitable){
                ...
                _data = Object.assign(_data, discountData);
            }
        }
