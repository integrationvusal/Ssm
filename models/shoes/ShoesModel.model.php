...
class ShoesModel extends CRUDModel implements GTDAO{

    public static $tableName = "vl1_ShoesModel";
    private static $typeIndex = 3;
    ...
    public static function __form($controller, $params = array()){
        ...
        return $controller::renderTemplate("goods" . ds . "shoes.tpl", $params, true);
    ...
    public static function createGT($array, $db){
        ...
        try{
            ...
            $imagePath = Utils::uploadImages("public" . ds . "user" . $array['user_id'] . ds . "goods_shoes", "image");

            $stmt = $db->prepare("INSERT INTO " . self::$tableName . "(user_id, code, category_id, `name`, brand, model, country, `type`, color, `size`, image, description)
             VALUES(:user_id, :code, :category_id,:name, :brand, :model, :country, :type, :color, :size, :image, :description)");
            ...
            $stmt->bindValue(":size", $array['size']);
    ...
    public static function updateGT($array, $db){
        ...
        try{
            ...
            $imagePath = Utils::uploadImages("public" . ds . "user" . $array['user_id'] . ds . "goods_clothes", "image");
            $imagePath .= $oldImages;
            ...
            color=:color, `size`=:size, image=:image, description=:description
    ...
    public static function getOne($array, $db){
        ...
        try{
            ...
            $stmt = $db->prepare("SELECT cat.name AS category, obj.*, concat(obj.code, ', ',obj.`name`, ', ',obj.brand, ', ',obj.model, ', ',obj.`size`) AS str FROM " . self::$tableName . " AS obj
    ...
    public static function getByCode($array, $db){
        try{
            ...
            $sql = "SELECT goods.id AS gid, goods.buy_price, goods.barcode, goods.sell_price, obj.*,concat(obj.code, ', ',obj.`name`, ', ',obj.brand, ', ',obj.model, ', ',obj.`size`) as str
